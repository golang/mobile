...
func goIOSBind(gobind string, pkgs []*packages.Package, targetPlatforms, targetArchs []string) error {
	var name string
	var title string
	...
	for _, platform := range targetPlatforms {
		...
		for _, arch := range targetArchs {
			...
			path, err := goIOSBindArchive(name+"-"+platform+"-"+arch, env, outSrcDir)
			if err != nil {
