...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		buf.Reset()
		w, closer = writer(filepath.Join("src", "gobind", pname+"_darwin.c"))
		processErr(g.GenC())
		io.Copy(w, &buf)
		closer()
		buf.Reset()
...
func genJavaPackages(dir string, classes []*java.Class, embedders []importers.Struct) error {
	...
	buf.Reset()
	cg.GenGo()
	if err := ioutil.WriteFile(filepath.Join(goBase, "classes_darwin.go"), buf.Bytes(), 0600); err != nil {
		return err
	}
	buf.Reset()
	cg.GenH()
	...
	buf.Reset()
	cg.GenC()
	if err := ioutil.WriteFile(filepath.Join(goBase, "classes_darwin.c"), buf.Bytes(), 0600); err != nil {
		return err
	}
	return nil
}
