...
func (g *goGen) genFunc(o *types.Func) {
	g.Printf("func proxy_%s(out, in *_seq.Buffer) {\n", o.Name())
	g.Indent()
...
func (g *goGen) genStruct(obj *types.TypeName, T *types.Struct) {
	...
	for _, f := range fields {
		g.Printf("func proxy%s_%s_Set(out, in *_seq.Buffer) {\n", obj.Name(), f.Name())
		g.Indent()
	...
	for _, m := range methods {
		g.Printf("func proxy%s_%s(out, in *_seq.Buffer) {\n", obj.Name(), m.Name())
		g.Indent()
		g.Printf("ref := in.ReadRef()\n")
	...
	for _, f := range fields {
		n := f.Name()
		g.Printf("_seq.Register(proxy%s_Descriptor, proxy%s_%s_Set_Code, proxy%s_%s_Set)\n", obj.Name(), obj.Name(), n, obj.Name(), n)
		g.Printf("_seq.Register(proxy%s_Descriptor, proxy%s_%s_Get_Code, proxy%s_%s_Get)\n", obj.Name(), obj.Name(), n, obj.Name(), n)
	}
	for _, m := range methods {
		n := m.Name()
		g.Printf("_seq.Register(proxy%s_Descriptor, proxy%s_%s_Code, proxy%s_%s)\n", obj.Name(), obj.Name(), n, obj.Name(), n)
	}
	g.Outdent()
	g.Printf("}\n\n")
}
...
func (g *goGen) genVar(o *types.Var) {
	...
	// var I int
	//
	// func var_setI(out, in *_seq.Buffer)
	g.Printf("func var_set%s(out, in *_seq.Buffer) {\n", o.Name())
	g.Indent()
	...
	// func var_getI(out, in *_seq.Buffer)
	g.Printf("func var_get%s(out, in *_seq.Buffer) {\n", o.Name())
	g.Indent()
...
func (g *goGen) genInterface(obj *types.TypeName) {
	...
	for i := 0; i < iface.NumMethods(); i++ {
		...
		g.Printf("in := new(_seq.Buffer)\n")
		for i := 0; i < params.Len(); i++ {
			g.genWrite(paramName(params, i), "in", params.At(i).Type())
		}

		if res.Len() == 0 {
			g.Printf("_seq.Transact((*_seq.Ref)(p), %q, proxy%s_%s_Code, in)\n", ifaceDesc, obj.Name(), m.Name())
		} else {
			g.Printf("out := _seq.Transact((*_seq.Ref)(p), %q, proxy%s_%s_Code, in)\n", ifaceDesc, obj.Name(), m.Name())
			var rvs []string
			for i := 0; i < res.Len(); i++ {
...
func (g *goGen) genRead(valName, seqName string, typ types.Type) {
	...
	case *types.Named:
		...
		case *types.Interface, *types.Pointer:
			...
			g.Printf("   %s = %s_ref.Get().(%s.%s)\n", valName, valName, g.pkg.Name(), o.Name())
			if hasProxy {
				g.Printf("} else if %s_ref.Num != _seq.NullRefNum {  // foreign object \n", valName)
...
func (g *goGen) gen() error {
	...
	if len(funcs) > 0 {
		...
		for i, name := range funcs {
			g.Printf("_seq.Register(%q, %d, proxy_%s)\n", g.pkg.Name(), i+1, name)
		}
		g.Outdent()
		g.Printf("}\n")
	}
	...
	if len(vars) > 0 {
		...
		for _, name := range vars {
			...
			g.Printf("_seq.Register(%q, %d, var_set%s)\n", varDesc, 1, name)
			g.Printf("_seq.Register(%q, %d, var_get%s)\n", varDesc, 2, name)
		}
		g.Outdent()
		g.Printf("}\n")
	}
